...
execFileSync('cargo', ['build', '--release', '-p', 'imprenta-pdf-wasm', '--target', TARGET], {
  cwd: root,
  stdio: 'inherit',
  env: { ...process.env, RUSTFLAGS: [process.env.RUSTFLAGS, ...flags].filter(Boolean).join(' ') },
});

const built = join(root, 'target', TARGET, 'release', 'imprenta_pdf_wasm.wasm');
const out = join(here, '..', 'imprenta-pdf.wasm');
