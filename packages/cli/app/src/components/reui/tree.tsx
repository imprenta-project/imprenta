...
/**
 * As much of a `useTree` instance as this file touches.
 *
 * The registry ships this as `any`. Structural typing accepts the real
 * instance either way, and `any` here would let a rename upstream go
 * unnoticed until the drag line stopped drawing.
 */
...
function TreeItem<T = unknown>({ item, className, render, children, ...props }: TreeItemProps<T>) {
  ...
  return (
    // One React context serves every element type in the tree, so it is typed
    // at `unknown` and read back through `useTreeContext<T>`. `ItemInstance` is
    // invariant in T, which is why this crossing needs saying out loud.
